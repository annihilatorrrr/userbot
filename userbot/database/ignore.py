...
class Ignore:
    ...
    def ignore_user(self, user_id):
        self.ignore_table.insert_one(
            {"user_id": user_id, "ignored": True, "last_message_time": None}
        )
    ...
    def is_ignored(self, user_id):
        return (
            self.ignore_table.find_one({"user_id": user_id, "ignored": True})
            is not None
        )

    def set_last_message_time(self, user_id):
        self.ignore_table.update_one(
            {"user_id": user_id}, {"$set": {"last_message_time": datetime.now()}}
        )
